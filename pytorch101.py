...
def create_sample_tensor() -> Tensor:
    """
    Return a torch Tensor of shape (5, 3) which is filled with zeros, except
    two entries: entry (0, 1) which is set to 10 and entry (1, 0) which is set to
    100.

    Returns:
        Tensor of shape (5, 3) as described above.
    """
    x = torch.zeros(5,3)
    ##########################################################################
...
def create_tensor_of_pi(M: int, N: int) -> Tensor:
    """
    Returns a Tensor of shape (M, N) filled entirely with the value 3.1415

    Args:
        M, N: Positive integers giving the shape of Tensor to create

    Returns:
        x: A tensor of shape (M, N) filled with the value 3.1415
    """
    x = torch.full((M, N), 3.1415)
    ##########################################################################
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    """
    assert start <= stop
    x = [x for x in range(start, stop+1) if x % 10 == 0]
...
def shuffle_cols(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = torch.zeros(x.shape[0], 4)  # Initialize y with zeros
    y[:, :2] = x[:,[0]] # assigns the first two columns of y to be the copies of the first column of x
    y[:, 2] = x[:, 2] # assigns the third column of y to be the same as the third column of x
    y[:, 3] = x[:, 1] # assigns the fourth column of y to be the same as the second column of x
...
def take_one_elem_per_col(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = torch.tensor([
    x[1, 0],  # Second element of the first column (x[1, 0])
    x[0, 1],  # First element of the second column (x[0, 1])
    x[3, 2]   # Fourth element of the third column (x[3, 2])
    ])
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    # Replace "pass" statement with your code
    num_classes = max(x) + 1
    y = torch.zeros(len(x), num_classes)
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    sigma_j = ((x - mu_j) ** 2).sum(dim=0) / (x.shape[0]-1)
    y = (x - mu_j) / sigma_j.sqrt()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
